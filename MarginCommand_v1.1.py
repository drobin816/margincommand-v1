...
def build_service_chart(model: dict, open_t: time) -> go.Figure:
    ...
    def bucket_to_label(bucket_idx: float) -> str:
        total_minutes = open_minutes + bucket_idx * bucket_size_hrs * 60
    ...
    # Show operators cleaner time intervals on the x-axis
    # If using 15-minute internal buckets, label every 30 minutes.
    label_every = 2 if bucket_size_hrs <= 0.26 else 1
    tickvals = [i for i in x_boundary if i % label_every == 0]
